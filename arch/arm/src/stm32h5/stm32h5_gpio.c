/****************************************************************************
 * arch/arm/src/stm32h5/stm32h5_gpio.c
 *
...
void stm32h5_gpioinit(void)
{
}

/****************************************************************************
 * Name: stm32h5_configgpio
 *
 * Description:
 *   Configure a GPIO pin based on bit-encoded description of the pin.
 *   Once it is configured as Alternative (GPIO_ALT|GPIO_CNF_AFPP|...)
 *   function, it must be unconfigured with stm32h5_unconfiggpio() with
...
int stm32h5_configgpio(uint32_t cfgset)
{
  uintptr_t base;
  uint32_t regval;
  uint32_t setting;
  ...
  switch (cfgset & GPIO_MODE_MASK)
    {
      ...
      case GPIO_OUTPUT:     /* General purpose output mode */
        ...
        stm32h5_gpiowrite(cfgset, (cfgset & GPIO_OUTPUT_SET) != 0);
  ...
  regval  = getreg32(base + STM32H5_GPIO_MODER_OFFSET);
  regval &= ~GPIO_MODER_MASK(pin);
  regval |= ((uint32_t)pinmode << GPIO_MODER_SHIFT(pin));
  putreg32(regval, base + STM32H5_GPIO_MODER_OFFSET);
  ...
  regval  = getreg32(base + STM32H5_GPIO_PUPDR_OFFSET);
  regval &= ~GPIO_PUPDR_MASK(pin);
  regval |= (setting << GPIO_PUPDR_SHIFT(pin));
  putreg32(regval, base + STM32H5_GPIO_PUPDR_OFFSET);
  ...
  if (pin < 8)
    {
      regoffset = STM32H5_GPIO_AFRL_OFFSET;
      pos       = pin;
    }
  else
    {
      regoffset = STM32H5_GPIO_AFRH_OFFSET;
      pos       = pin - 8;
    }
  ...
  regval  = getreg32(base + STM32H5_GPIO_OSPEED_OFFSET);
  regval &= ~GPIO_OSPEED_MASK(pin);
  regval |= (setting << GPIO_OSPEED_SHIFT(pin));
  putreg32(regval, base + STM32H5_GPIO_OSPEED_OFFSET);

  /* Set push-pull/open-drain (Only outputs and alternate function pins) */

  regval  = getreg32(base + STM32H5_GPIO_OTYPER_OFFSET);
  ...
  putreg32(regval, base + STM32H5_GPIO_OTYPER_OFFSET);
...
int stm32h5_unconfiggpio(uint32_t cfgset)
{
  ...
  return stm32h5_configgpio(cfgset);
}

/****************************************************************************
 * Name: stm32h5_gpiowrite
 *
 * Description:
...
void stm32h5_gpiowrite(uint32_t pinset, bool value)
{
  uint32_t base;
  uint32_t bit;
  unsigned int port;
  ...
  if (port < STM32H5_NPORTS)
    {
      ...
      putreg32(bit, base + STM32H5_GPIO_BSRR_OFFSET);
    }
}

/****************************************************************************
 * Name: stm32h5_gpioread
 *
 * Description:
...
bool stm32h5_gpioread(uint32_t pinset)
{
  uint32_t base;
  unsigned int port;
  unsigned int pin;
  ...
  if (port < STM32H5_NPORTS)
    {
      ...
      pin = (pinset & GPIO_PIN_MASK) >> GPIO_PIN_SHIFT;
      return ((getreg32(base + STM32H5_GPIO_IDR_OFFSET) & (1 << pin)) != 0);
    }
