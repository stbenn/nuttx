/****************************************************************************
 * arch/arm/src/stm32h5/stm32h5_serial.c
 *
 ...
 *
 * See stm32h5serial_restoreusartint where the masking is done.
 */
...
static void stm32h5serial_setformat(struct uart_dev_s *dev);
#endif
static int  stm32h5serial_setup(struct uart_dev_s *dev);
static void stm32h5serial_shutdown(struct uart_dev_s *dev);
static int  stm32h5serial_attach(struct uart_dev_s *dev);
static void stm32h5serial_detach(struct uart_dev_s *dev);
static int  stm32h5serial_interrupt(int irq, void *context,
                                    void *arg);
static int  stm32h5serial_ioctl(struct file *filep, int cmd,
                                unsigned long arg);
#ifndef SERIAL_HAVE_ONLY_DMA
static int  stm32h5serial_receive(struct uart_dev_s *dev,
                                  unsigned int *status);
static void stm32h5serial_rxint(struct uart_dev_s *dev, bool enable);
static bool stm32h5serial_rxavailable(struct uart_dev_s *dev);
#endif
#ifdef CONFIG_SERIAL_IFLOWCONTROL
static bool stm32h5serial_rxflowcontrol(struct uart_dev_s *dev,
                                        unsigned int nbuffered, bool upper);
#endif
static void stm32h5serial_send(struct uart_dev_s *dev, int ch);
static void stm32h5serial_txint(struct uart_dev_s *dev, bool enable);
static bool stm32h5serial_txready(struct uart_dev_s *dev);

#ifdef SERIAL_HAVE_DMA
static int  stm32h5serial_dmasetup(struct uart_dev_s *dev);
static void stm32h5serial_dmashutdown(struct uart_dev_s *dev);
static int  stm32h5serial_dmareceive(struct uart_dev_s *dev,
                                     unsigned int *status);
static void stm32h5serial_dmareenable(struct stm32h5_serial_s *priv);
#ifdef CONFIG_SERIAL_IFLOWCONTROL
static bool stm32h5serial_dmaiflowrestart(struct stm32h5_serial_s *priv);
#endif
static void stm32h5serial_dmarxint(struct uart_dev_s *dev, bool enable);
static bool stm32h5serial_dmarxavailable(struct uart_dev_s *dev);

static void stm32h5serial_dmarxcallback(DMA_HANDLE handle, uint8_t status,
                                        void *arg);
#endif

#ifdef CONFIG_PM
static void stm32h5serial_setsuspend(struct uart_dev_s *dev, bool suspend);
static void stm32h5serial_pm_setsuspend(bool suspend);
static void stm32h5serial_pmnotify(struct pm_callback_s *cb, int domain,
                                   enum pm_state_e pmstate);
static int  stm32h5serial_pmprepare(struct pm_callback_s *cb, int domain,
                                    enum pm_state_e pmstate);
#endif
...
{
  .setup          = stm32h5serial_setup,
  .shutdown       = stm32h5serial_shutdown,
  .attach         = stm32h5serial_attach,
  .detach         = stm32h5serial_detach,
  .ioctl          = stm32h5serial_ioctl,
  .receive        = stm32h5serial_receive,
  .rxint          = stm32h5serial_rxint,
  .rxavailable    = stm32h5serial_rxavailable,
#  ifdef CONFIG_SERIAL_IFLOWCONTROL
  .rxflowcontrol  = stm32h5serial_rxflowcontrol,
#  endif
  .send           = stm32h5serial_send,
  .txint          = stm32h5serial_txint,
  .txready        = stm32h5serial_txready,
  .txempty        = stm32h5serial_txready,
};
#endif
...
{
  .setup          = stm32h5serial_dmasetup,
  .shutdown       = stm32h5serial_dmashutdown,
  .attach         = stm32h5serial_attach,
  .detach         = stm32h5serial_detach,
  .ioctl          = stm32h5serial_ioctl,
  .receive        = stm32h5serial_dmareceive,
  .rxint          = stm32h5serial_dmarxint,
  .rxavailable    = stm32h5serial_dmarxavailable,
#  ifdef CONFIG_SERIAL_IFLOWCONTROL
  .rxflowcontrol  = stm32h5serial_rxflowcontrol,
#  endif
  .send           = stm32h5serial_send,
...
#ifdef CONFIG_STM32H5_LPUART1_SERIALDRIVER
static struct stm32h5_serial_s g_lpuart1priv =
{
  .dev =
    {
#  if CONSOLE_UART == 1
...
#ifdef CONFIG_STM32H5_USART1_SERIALDRIVER
static struct stm32h5_serial_s g_usart1priv =
{
  .dev =
    {
#  if CONSOLE_UART == 2
...
#ifdef CONFIG_STM32H5_USART2_SERIALDRIVER
static struct stm32h5_serial_s g_usart2priv =
{
  .dev =
    {
#  if CONSOLE_UART == 3
...
#ifdef CONFIG_STM32H5_USART3_SERIALDRIVER
static struct stm32h5_serial_s g_usart3priv =
{
  .dev =
    {
#  if CONSOLE_UART == 4
...
#ifdef CONFIG_STM32H5_UART4_SERIALDRIVER
static struct stm32h5_serial_s g_uart4priv =
{
  .dev =
    {
#  if CONSOLE_UART == 5
...
#ifdef CONFIG_STM32H5_UART5_SERIALDRIVER
static struct stm32h5_serial_s g_uart5priv =
{
  .dev =
    {
#  if CONSOLE_UART == 6
...
{
  .pm_cb.notify  = stm32h5serial_pmnotify,
  .pm_cb.prepare = stm32h5serial_pmprepare,
...
/****************************************************************************
 * Name: stm32h5serial_getreg
 ****************************************************************************/

static inline
uint32_t stm32h5serial_getreg(struct stm32h5_serial_s *priv, int offset)
{
  return getreg32(priv->usartbase + offset);
}

/****************************************************************************
 * Name: stm32h5serial_putreg
 ****************************************************************************/

static inline
void stm32h5serial_putreg(struct stm32h5_serial_s *priv,
...
/****************************************************************************
 * Name: stm32h5serial_setusartint
 ****************************************************************************/

static inline
void stm32h5serial_setusartint(struct stm32h5_serial_s *priv,
                               uint16_t ie)
{
  uint32_t cr;
  ...
  cr = stm32h5serial_getreg(priv, STM32_USART_CR1_OFFSET);
  cr &= ~(USART_CR1_USED_INTS);
  cr |= (ie & (USART_CR1_USED_INTS));
  stm32h5serial_putreg(priv, STM32_USART_CR1_OFFSET, cr);

  cr = stm32h5serial_getreg(priv, STM32_USART_CR3_OFFSET);
  cr &= ~USART_CR3_EIE;
  cr |= (ie & USART_CR3_EIE);
  stm32h5serial_putreg(priv, STM32_USART_CR3_OFFSET, cr);
}
...
static void stm32h5serial_restoreusartint(struct stm32h5_serial_s *priv,
                                          uint16_t ie)
{
  irqstate_t flags;
  ...
  stm32h5serial_setusartint(priv, ie);

  spin_unlock_irqrestore(NULL, flags);
}

/****************************************************************************
 * Name: stm32h5serial_disableusartint
 ****************************************************************************/

static void stm32h5serial_disableusartint(struct stm32h5_serial_s *priv,
                                          uint16_t *ie)
{
  irqstate_t flags;
  ...
  if (ie)
    {
      ...
      cr1 = stm32h5serial_getreg(priv, STM32_USART_CR1_OFFSET);
      cr3 = stm32h5serial_getreg(priv, STM32_USART_CR3_OFFSET);
  ...
  stm32h5serial_setusartint(priv, 0);

  spin_unlock_irqrestore(NULL, flags);
}

/****************************************************************************
 * Name: stm32h5serial_dmanextrx
 *
 * Description:
...
#ifdef SERIAL_HAVE_DMA
static int stm32h5serial_dmanextrx(struct stm32h5_serial_s *priv)
{
  size_t dmaresidual;

  dmaresidual = stm32h5_dmaresidual(priv->rxdma);
...
static void stm32h5serial_setformat(struct uart_dev_s *dev)
{
  ...
  stm32h5serial_putreg(priv, STM32_USART_CR1_OFFSET, cr1);
  stm32h5serial_putreg(priv, STM32_USART_BRR_OFFSET, brr);

  /* Configure parity mode */

  regval  = stm32h5serial_getreg(priv, STM32_USART_CR1_OFFSET);
  ...
  stm32h5serial_putreg(priv, STM32_USART_CR1_OFFSET, regval);

  /* Configure STOP bits */

  regval = stm32h5serial_getreg(priv, STM32_USART_CR2_OFFSET);
  ...
  stm32h5serial_putreg(priv, STM32_USART_CR2_OFFSET, regval);

  /* Configure hardware flow control */

  regval  = stm32h5serial_getreg(priv, STM32_USART_CR3_OFFSET);
  ...
  stm32h5serial_putreg(priv, STM32_USART_CR3_OFFSET, regval);
}
#endif /* CONFIG_SUPPRESS_UART_CONFIG */

/****************************************************************************
 * Name: stm32h5serial_setsuspend
 *
 * Description:
...
static void stm32h5serial_setsuspend(struct uart_dev_s *dev, bool suspend)
{
  struct stm32h5_serial_s *priv = (struct stm32h5_serial_s *)dev->priv;
#ifdef SERIAL_HAVE_DMA
  ...
  if (suspend)
    {
      ...
      if (priv->iflow)
        {
          ...
          stm32h5_configgpio((priv->rts_gpio & ~GPIO_MODE_MASK)
      ...
      stm32h5serial_disableusartint(priv, &priv->suspended_ie);

      /* Wait last Tx to complete. */

      while ((stm32h5serial_getreg(priv, STM32_USART_ISR_OFFSET) &
              USART_ISR_TC) == 0);
  ...
  else
    {
      ...
      if (priv->dev.ops == &g_uart_dma_ops && priv->rxdmasusp)
        {
          ...
          if (priv->iflow)
            {
              stm32h5serial_dmaiflowrestart(priv);
            }
          else
#endif
            {
              ...
              stm32h5serial_dmareenable(priv);
      ...
      stm32h5serial_restoreusartint(priv, priv->suspended_ie);
      ...
      if (priv->iflow)
        {
          ...
          stm32h5_configgpio(priv->rts_gpio);
        }
#endif
    }
  ...
  if (dmarestored)
    {
      ...
      if (priv->rxdma != NULL)
        {
          stm32h5serial_dmarxcallback(priv->rxdma, 0, priv);
        }
...
/****************************************************************************
 * Name: stm32h5serial_pm_setsuspend
 *
 * Description:
...
static void stm32h5serial_pm_setsuspend(bool suspend)
{
  ...
  for (n = 0; n < STM32H5_NLPUART + STM32H5_NUSART + STM32H5_NUART; n++)
    {
      struct stm32h5_serial_s *priv = g_uart_devs[n];
      ...
      stm32h5serial_setsuspend(&priv->dev, suspend);
    }
}
#endif

/****************************************************************************
 * Name: stm32h5serial_setapbclock
 *
 * Description:
...
static void stm32h5serial_setapbclock(struct uart_dev_s *dev, bool on)
{
  struct stm32h5_serial_s *priv =
    (struct stm32h5_serial_s *)dev->priv;
  uint32_t rcc_en;
...
static int stm32h5serial_setup(struct uart_dev_s *dev)
{
   ...
   * was enabled in stm32h5_lowsetup().
   */

  /* Enable USART APB1/2 clock */

  stm32h5serial_setapbclock(dev, true);

  /* Configure pins for USART use */

  stm32h5_configgpio(priv->tx_gpio);
  stm32h5_configgpio(priv->rx_gpio);

#ifdef CONFIG_SERIAL_OFLOWCONTROL
  if (priv->cts_gpio != 0)
    {
      stm32h5_configgpio(priv->cts_gpio);
    }
#endif
  ...
  if (priv->rs485_dir_gpio != 0)
    {
      stm32h5_configgpio(priv->rs485_dir_gpio);
      stm32h5_gpiowrite(priv->rs485_dir_gpio, !priv->rs485_dir_polarity);
    }
#endif
...
static int stm32h5serial_dmasetup(struct uart_dev_s *dev)
{
  ...
  if (!dev->isconsole)
    {
      result = stm32h5serial_setup(dev);
      if (result != OK)
        {
          return result;
        }
  ...
  if (priv->iflow)
    {
      ...
      stm32h5_dmastart(priv->rxdma, stm32h5serial_dmarxcallback,
                       (void *)priv, false);
    }
  else
#endif
...
static void stm32h5serial_shutdown(struct uart_dev_s *dev)
{
  ...
  stm32h5serial_disableusartint(priv, NULL);

  /* Disable USART APB1/2 clock */

  stm32h5serial_setapbclock(dev, false);
  ...
  stm32h5_unconfiggpio(priv->tx_gpio);
  stm32h5_unconfiggpio(priv->rx_gpio);

#ifdef CONFIG_SERIAL_OFLOWCONTROL
  if (priv->cts_gpio != 0)
    {
      stm32h5_unconfiggpio(priv->cts_gpio);
    }
#endif
  ...
  if (priv->rts_gpio != 0)
    {
      stm32h5_unconfiggpio(priv->rts_gpio);
    }
#endif
  ...
  if (priv->rs485_dir_gpio != 0)
    {
      stm32h5_unconfiggpio(priv->rs485_dir_gpio);
    }
#endif
}

/****************************************************************************
 * Name: stm32h5serial_dmashutdown
 *
 * Description:
 *   Disable the USART.  This method is called when the serial
 *   port is closed
...
static int stm32h5serial_attach(struct uart_dev_s *dev)
{
  ...
  ret = irq_attach(priv->irq, stm32h5serial_interrupt, priv);
...
static int stm32h5serial_interrupt(int irq, void *context, void *arg)
{
  struct stm32h5_serial_s *priv = (struct stm32h5_serial_s *)arg;
  int  passes;
  bool handled;
  ...
  for (passes = 0; passes < 256 && handled; passes++)
    {
      ...
      priv->sr = stm32h5serial_getreg(priv, STM32_USART_ISR_OFFSET);
      ...
      if ((priv->sr & USART_ISR_TC) != 0 &&
        ...
        {
          stm32h5_gpiowrite(priv->rs485_dir_gpio, !priv->rs485_dir_polarity);
          stm32h5serial_restoreusartint(priv, priv->ie & ~USART_CR1_TCIE);
        }
#endif
       ...
       */
               ...
               != 0)
        {
          ...
          stm32h5serial_putreg(priv, STM32_USART_ICR_OFFSET,
                               ...
                               unsigned long arg)
{
  ...
  switch (cmd)
    ...
    case TIOCSERGSTRUCT:
      {
        struct stm32h5_serial_s *user;

        user = (struct stm32h5_serial_s *)arg;
        ...
        if (!user)
          ...
          {
            memcpy(user, dev, sizeof(struct stm32h5_serial_s));
          }
      }
      break;
#endif
    ...
    case TIOCSSINGLEWIRE:
      {
        ...
        uint32_t cr = stm32h5serial_getreg(priv, STM32_USART_CR3_OFFSET);
        ...
        stm32h5serial_putreg(priv, STM32_USART_CR1_OFFSET, cr1 | cr1_ue);
        leave_critical_section(flags);
      }
     break;
#endif
    ...
    case TIOCSINVERT:
      {
        ...
        uint32_t cr = stm32h5serial_getreg(priv, STM32_USART_CR2_OFFSET);
        ...
        stm32h5serial_putreg(priv, STM32_USART_CR2_OFFSET, cr);
    ...
    case TIOCSBRK:  /* No BSD compatibility: Turn break on for M bit times */
      {
        ...
        flags = enter_critical_section();
        cr1   = stm32h5serial_getreg(priv, STM32_USART_CR1_OFFSET);
        stm32h5serial_putreg(priv, STM32_USART_CR1_OFFSET,
                             cr1 | USART_CR1_SBK);
        leave_critical_section(flags);
      }
      break;
                                 ...
                                 unsigned int *status)
{
  ...
  rdr      = stm32h5serial_getreg(priv, STM32_USART_RDR_OFFSET);
...
static void stm32h5serial_rxint(struct uart_dev_s *dev, bool enable)
{
  ...
  stm32h5serial_restoreusartint(priv, ie);
  leave_critical_section(flags);
}
#endif

/****************************************************************************
 * Name: stm32h5serial_rxavailable
 *
 * Description:
...
static bool stm32h5serial_rxavailable(struct uart_dev_s *dev)
{
  ...
  return ((stm32h5serial_getreg(priv, STM32_USART_ISR_OFFSET) &
           USART_ISR_RXNE) != 0);
}
#endif

/****************************************************************************
 * Name: stm32h5serial_rxflowcontrol
 *
 * Description:
  ...
  if (priv->iflow && (priv->rts_gpio != 0))
    {
      ...
      stm32h5_gpiowrite(priv->rts_gpio, upper);
                                    ...
                                    unsigned int *status)
{
  ...
  if (stm32h5serial_dmanextrx(priv) != priv->rxdmanext)
    {
...
/****************************************************************************
 * Name: stm32h5serial_dmareenable
 *
 * Description:
...
/****************************************************************************
 * Name: stm32h5serial_dmaiflowrestart
 *
 * Description:
      ...
      if (priv->rxdmasusp)
        {
          ...
          if (stm32h5serial_dmarxavailable(&priv->dev))
            {
...
/****************************************************************************
 * Name: stm32h5serial_dmarxavailable
 *
 * Description:
 *   Return true if the receive register is not empty
 *
...
static bool stm32h5serial_dmarxavailable(struct uart_dev_s *dev)
{
  ...
  return (stm32h5serial_dmanextrx(priv) != priv->rxdmanext);
}
#endif

/****************************************************************************
 * Name: stm32h5serial_send
 *
 * Description:
...
static void stm32h5serial_send(struct uart_dev_s *dev, int ch)
{
  ...
  stm32h5serial_putreg(priv, STM32_USART_TDR_OFFSET, (uint32_t)ch);
}

/****************************************************************************
 * Name: stm32h5serial_txint
 *
 * Description:
...
#  endif
  ...
  else
    {
      ...
      stm32h5serial_restoreusartint(priv, priv->ie & ~USART_CR1_TXEIE);
    }
...
/****************************************************************************
 * Name: stm32h5serial_dmarxcallback
 *
 * Description:
                                        ...
                                        void *arg)
{
  ...
  if (priv->rxenable && stm32h5serial_dmarxavailable(&priv->dev))
    {
      uart_recvchars(&priv->dev);
  ...
  switch (pmstate)
    {
      case PM_NORMAL:
        {
          stm32h5serial_pm_setsuspend(false);
        }
        break;
      ...
      case PM_STANDBY:
        {
          ...
          stm32h5serial_pm_setsuspend(true);
        }
        break;
  ...
  for (i = 0; i < STM32H5_NLPUART + STM32H5_NUSART + STM32H5_NUART; i++)
    {
      if (g_uart_devs[i])
        {
          stm32h5serial_disableusartint(g_uart_devs[i], NULL);
        }
    }
...
#if CONSOLE_UART > 0
  stm32h5serial_setup(&g_uart_devs[CONSOLE_UART - 1]->dev);
#endif
#endif /* HAVE UART */
}
...
void arm_serialinit(void)
{
  ...
  stm32h5serial_dmasetup(&g_uart_devs[CONSOLE_UART - 1]->dev);
#endif
...
#ifdef SERIAL_HAVE_DMA
void stm32h5_serial_dma_poll(void)
{
    irqstate_t flags;
  ...
  if (g_lpuart1priv.rxdma != NULL)
    {
      stm32h5serial_dmarxcallback(g_lpuart1priv.rxdma, 0, &g_lpuart1priv);
    }
#endif
  ...
  if (g_usart1priv.rxdma != NULL)
    {
      stm32h5serial_dmarxcallback(g_usart1priv.rxdma, 0, &g_usart1priv);
    }
#endif
  ...
  if (g_usart2priv.rxdma != NULL)
    {
      stm32h5serial_dmarxcallback(g_usart2priv.rxdma, 0, &g_usart2priv);
    }
#endif
  ...
  if (g_usart3priv.rxdma != NULL)
    {
      stm32h5serial_dmarxcallback(g_usart3priv.rxdma, 0, &g_usart3priv);
    }
#endif
  ...
  if (g_uart4priv.rxdma != NULL)
    {
      stm32h5serial_dmarxcallback(g_uart4priv.rxdma, 0, &g_uart4priv);
    }
#endif
  ...
  if (g_uart5priv.rxdma != NULL)
    {
      stm32h5serial_dmarxcallback(g_uart5priv.rxdma, 0, &g_uart5priv);
    }
#endif
...
{
#if CONSOLE_UART > 0
  struct stm32h5_serial_s *priv = g_uart_devs[CONSOLE_UART - 1];
  uint16_t ie;

  stm32h5serial_disableusartint(priv, &ie);
