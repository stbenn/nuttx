...
#  endif
  ...
  .usartbase     = STM32H5_LPUART1_BASE,
                               ...
                               uint16_t ie)
{
  ...
  cr = stm32h5serial_getreg(priv, STM32H5_USART_CR1_OFFSET);
  cr &= ~(USART_CR1_USED_INTS);
  cr |= (ie & (USART_CR1_USED_INTS));
  stm32h5serial_putreg(priv, STM32H5_USART_CR1_OFFSET, cr);

  cr = stm32h5serial_getreg(priv, STM32H5_USART_CR3_OFFSET);
  cr &= ~USART_CR3_EIE;
  cr |= (ie & USART_CR3_EIE);
  stm32h5serial_putreg(priv, STM32H5_USART_CR3_OFFSET, cr);
}
  ...
  if (ie)
    {
      ...
      cr3 = stm32h5serial_getreg(priv, STM32H5_USART_CR3_OFFSET);
...
static void stm32h5serial_setformat(struct uart_dev_s *dev)
{
  ...
  stm32h5serial_putreg(priv, STM32H5_USART_BRR_OFFSET, brr);
  ...
  stm32h5serial_putreg(priv, STM32H5_USART_CR2_OFFSET, regval);

  /* Configure hardware flow control */

  regval  = stm32h5serial_getreg(priv, STM32H5_USART_CR3_OFFSET);
  ...
  stm32h5serial_putreg(priv, STM32H5_USART_CR3_OFFSET, regval);
}
  ...
  if (suspend)
    {
      ...
      while ((stm32h5serial_getreg(priv, STM32H5_USART_ISR_OFFSET) &
              USART_ISR_TC) == 0);
  ...
  if (priv->iflow)
    {
      ...
      stm32h5_dmasetup(priv->rxdma,
                     priv->usartbase + STM32H5_USART_RDR_OFFSET,
  ...
  for (passes = 0; passes < 256 && handled; passes++)
    {
      ...
      priv->sr = stm32h5serial_getreg(priv, STM32H5_USART_ISR_OFFSET);
       ...
       */
               ...
               != 0)
        {
          ...
          stm32h5serial_putreg(priv, STM32H5_USART_ICR_OFFSET,
    ...
    case TIOCSSINGLEWIRE:
      {
        ...
        uint32_t cr = stm32h5serial_getreg(priv, STM32H5_USART_CR3_OFFSET);
        ...
        stm32h5serial_putreg(priv, STM32H5_USART_CR1_OFFSET, cr1 | cr1_ue);
        leave_critical_section(flags);
      }
     break;
#endif
    ...
    case TIOCSINVERT:
      {
        ...
        uint32_t cr = stm32h5serial_getreg(priv, STM32H5_USART_CR2_OFFSET);
        ...
        stm32h5serial_putreg(priv, STM32H5_USART_CR2_OFFSET, cr);
                                 ...
                                 unsigned int *status)
{
  ...
  rdr      = stm32h5serial_getreg(priv, STM32H5_USART_RDR_OFFSET);
...
static bool stm32h5serial_rxavailable(struct uart_dev_s *dev)
{
  ...
  return ((stm32h5serial_getreg(priv, STM32H5_USART_ISR_OFFSET) &
           USART_ISR_RXNE) != 0);
}
#endif
...
static void stm32h5serial_send(struct uart_dev_s *dev, int ch)
{
  ...
  stm32h5serial_putreg(priv, STM32H5_USART_TDR_OFFSET, (uint32_t)ch);
}
