 ...
 * RCC_CCIPR4_OCTOSPI1SEL_XXX values (XXX = HCLK, PLL1, PLL2, PER).
...
static struct stm32_qspidev_s g_qspi0dev =
{
  ...
  .base              = STM32_OCTOSPI1_BASE,
  .lock              = NXMUTEX_INITIALIZER,
#ifdef CONFIG_STM32H5_QSPI_INTERRUPTS
  .handler           = qspi0_interrupt,
  .irq               = STM32_IRQ_OCTOSPI1,
                           ...
                           uint8_t fctn)
{
  ...
  regval = (regval & ~(QSPI_TCR_DCYC_MASK)) |
            QSPI_TCR_DCYC(xctn->dummycycles);
  ...
  if (dest != NULL)
    {
      ...
      if (xctn->addrmode != CCR_ADMODE_NONE)
        ...
        {
          qspi_putreg(priv, xctn->instr, STM32_QUADSPI_IR_OFFSET);
        }

      if (qspi_getreg(priv, STM32_QUADSPI_SR_OFFSET) & QSPI_SR_TEF)
        {
          qspi_putreg(priv, QSPI_FCR_CTEF, STM32_QUADSPI_FCR_OFFSET);
          qspi_abort(priv);
          return -ENOTBLK;
        }
  ...
  if (mode != priv->mode)
    {
      ...
      regval  = qspi_getreg(priv, STM32_QUADSPI_DCR1_OFFSET);
      regval &= ~(QSPI_DCR1_CKMODE);
      ...
      qspi_putreg(priv, regval, STM32_QUADSPI_DCR1_OFFSET);
...
static int qspi_hw_initialize(struct stm32_qspidev_s *priv)
{
  ...
  regval &= ~(QSPI_CR_TEIE | QSPI_CR_TCIE | QSPI_CR_FTIE | QSPI_CR_SMIE |
              QSPI_CR_TOIE | QSPI_CR_MSEL | QSPI_CR_DMM);
  ...
  regval &= ~(QSPI_DCR1_CKMODE |
              QSPI_DCR1_CSHT_MASK |
              QSPI_DCR1_DEVSIZE_MASK);

  regval |= ((CONFIG_STM32H5_QSPI_CSHT - 1) << QSPI_DCR1_CSHT_SHIFT);
  if (0 != CONFIG_STM32H5_QSPI_FLASH_SIZE)
    {
      ...
      regval |= ((nlog2size - 1) << QSPI_DCR1_DEVSIZE_SHIFT);
    }
  ...
  if (intf == 0)
    {
      ...
      regval = getreg32(STM32_RCC_AHB4RSTR);
      regval |= RCC_AHB4RSTR_OSPI1RST;
      putreg32(regval, STM32_RCC_AHB4RSTR);
      regval &= ~RCC_AHB4RSTR_OSPI1RST;
      putreg32(regval, STM32_RCC_AHB4RSTR);
