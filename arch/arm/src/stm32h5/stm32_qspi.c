...
#  undef CONFIG_STM32H5_OCTOSPI_DMADEBUG
#endif
...
#if defined(CONFIG_STM32H5_OCTOSPI_INTERRUPTS) && defined(CONFIG_STM32H5_OCTOSPI_DMA)
...
#if (!defined(GPIO_OCTOSPI_CS) || !defined(GPIO_OCTOSPI_IO0) || !defined(GPIO_OCTOSPI_IO1) || \
    !defined(GPIO_OCTOSPI_IO2) || !defined(GPIO_OCTOSPI_IO3) || !defined(GPIO_OCTOSPI_SCK))
#  error you must define QSPI pinmapping options for GPIO_OCTOSPI_CS GPIO_OCTOSPI_IO0 \
    GPIO_OCTOSPI_IO1 GPIO_OCTOSPI_IO2 GPIO_OCTOSPI_IO3 GPIO_OCTOSPI_SCK in your board.h
#endif

#ifdef CONFIG_STM32H5_OCTOSPI_DMA

#  ifdef DMAMAP_OCTOSPI
...
#    define DMACHAN_OCTOSPI           DMAMAP_OCTOSPI
#  endif

#  if defined(CONFIG_STM32H5_OCTOSPI_DMAPRIORITY_LOW)
#    define OCTOSPI_DMA_PRIO  DMA_SCR_PRILO
#  elif defined(CONFIG_STM32H5_OCTOSPI_DMAPRIORITY_MEDIUM)
#    define OCTOSPI_DMA_PRIO  DMA_SCR_PRIMED
#  elif defined(CONFIG_STM32H5_OCTOSPI_DMAPRIORITY_HIGH)
#    define OCTOSPI_DMA_PRIO  DMA_SCR_PRIHI
#  elif defined(CONFIG_STM32H5_OCTOSPI_DMAPRIORITY_VERYHIGH)
#    define OCTOSPI_DMA_PRIO  DMA_SCR_PRIVERYHI
#  else
#    define OCTOSPI_DMA_PRIO  DMA_SCR_PRIMED
#  endif

#endif /* CONFIG_STM32H5_OCTOSPI_DMA */

#ifndef STM32_SYSCLK_FREQUENCY
#  error your board.h needs to define the value of STM32_SYSCLK_FREQUENCY
#endif

#if !defined(CONFIG_STM32H5_OCTOSPI_FLASH_SIZE) || 0 == CONFIG_STM32H5_OCTOSPI_FLASH_SIZE
#  error you must specify a positive flash size via CONFIG_STM32H5_OCTOSPI_FLASH_SIZE
#endif
...
/* The board.h file may choose a different clock source for OCTOSPI
 * peripheral by defining the BOARD_OCTOSPI_CLK macro to one of the
 * RCC_D1CCIPR_OCTOSPISEL_XXX values (XXX = HCLK, PLL1, PLL2, PER).
 * OCTOSPI clock defaults to HCLK.
 */

#ifndef BOARD_OCTOSPI_CLK
/* Clock OCTOSPI from HCLK by default */

#  define BOARD_OCTOSPI_CLK        RCC_D1CCIPR_OCTOSPISEL_HCLK
#endif
...
#if BOARD_OCTOSPI_CLK == RCC_D1CCIPR_OCTOSPISEL_HCLK
#  define OCTOSPI_CLK_FREQUENCY    STM32_HCLK_FREQUENCY
#elif BOARD_OCTOSPI_CLK == RCC_D1CCIPR_OCTOSPISEL_PLL1
#  define OCTOSPI_CLK_FREQUENCY    STM32_PLL1Q_FREQUENCY
#elif BOARD_OCTOSPI_CLK == RCC_D1CCIPR_OCTOSPISEL_PLL2
#  define OCTOSPI_CLK_FREQUENCY    STM32_PLL2R_FREQUENCY
#elif BOARD_OCTOSPI_CLK == RCC_D1CCIPR_OCTOSPISEL_PER
#  define OCTOSPI_CLK_FREQUENCY    STM32_PER_FREQUENCY
#else
#  error "BOARD_OCTOSPI_CLK has unknown value!"
#endif
...
#ifdef CONFIG_STM32H5_OCTOSPI_INTERRUPTS
...
#ifdef CONFIG_STM32H5_OCTOSPI_DMADEBUG
  struct stm32_dmaregs_s dmaregs[DMA_NSAMPLES];
#endif

#ifdef CONFIG_STM32H5_OCTOSPI_REGDEBUG
...
#  ifndef CONFIG_STM32H5_OCTOSPI_DMATHRESHOLD
#    define CONFIG_STM32H5_OCTOSPI_DMATHRESHOLD 4
#  endif
...
#ifdef CONFIG_OCTOSPI_HWFEATURES
  .hwfeatures        = NULL,
#endif
...
static struct stm32_qspidev_s g_qspi0dev =
{
  ...
  .base              = STM32_OCTOSPI_BASE,
  .lock              = NXMUTEX_INITIALIZER,
#ifdef CONFIG_STM32H5_OCTOSPI_INTERRUPTS
  .handler           = qspi0_interrupt,
  .irq               = STM32_IRQ_OCTOSPI,
...
static void qspi_dumpregs(struct stm32_qspidev_s *priv, const char *msg)
{
  ...
  regval = getreg32(priv->base + STM32_OCTOSPI_CR_OFFSET);    /* Control Register */
  spiinfo("CR:%08x\n", regval);
  spiinfo("  EN:%1d ABORT:%1d DMAEN:%1d TCEN:%1d SSHIFT:%1d\n"
          "  FTHRES: %d\n"
          "  TEIE:%1d TCIE:%1d FTIE:%1d SMIE:%1d TOIE:%1d APMS:%1d PMM:%1d\n"
          "  PRESCALER: %d\n",
          (regval & OCTOSPI_CR_EN) ? 1 : 0,
          (regval & OCTOSPI_CR_ABORT) ? 1 : 0,
          (regval & OCTOSPI_CR_DMAEN) ? 1 : 0,
          (regval & OCTOSPI_CR_TCEN) ? 1 : 0,
          (regval & OCTOSPI_CR_SSHIFT) ? 1 : 0,
          (regval & OCTOSPI_CR_FTHRES_MASK) >> OCTOSPI_CR_FTHRES_SHIFT,
          (regval & OCTOSPI_CR_TEIE) ? 1 : 0,
          (regval & OCTOSPI_CR_TCIE) ? 1 : 0,
          (regval & OCTOSPI_CR_FTIE) ? 1 : 0,
          (regval & OCTOSPI_CR_SMIE) ? 1 : 0,
          (regval & OCTOSPI_CR_TOIE) ? 1 : 0,
          (regval & OCTOSPI_CR_APMS) ? 1 : 0,
          (regval & OCTOSPI_CR_PMM) ? 1 : 0,
          (regval & OCTOSPI_CR_PRESCALER_MASK) >> OCTOSPI_CR_PRESCALER_SHIFT);

  regval = getreg32(priv->base + STM32_OCTOSPI_DCR_OFFSET);   /* Device Configuration Register */
  spiinfo("DCR:%08x\n", regval);
  spiinfo("  CKMODE:%1d CSHT:%d FSIZE:%d\n",
          (regval & OCTOSPI_DCR_CKMODE) ? 1 : 0,
          (regval & OCTOSPI_DCR_CSHT_MASK) >> OCTOSPI_DCR_CSHT_SHIFT,
          (regval & OCTOSPI_DCR_FSIZE_MASK) >> OCTOSPI_DCR_FSIZE_SHIFT);

  regval = getreg32(priv->base + STM32_OCTOSPI_CCR_OFFSET);   /* Communication Configuration Register */
  spiinfo("CCR:%08x\n", regval);
  spiinfo("   INST:%02x IMODE:%d ADMODE:%d ADSIZE:%d ABMODE:%d\n"
          "   ABSIZE:%d DCYC:%d DMODE:%d FMODE:%d\n"
          "   SIOO:%1d DDRM:%1d\n",
          (regval & OCTOSPI_CCR_INSTRUCTION_MASK) >> OCTOSPI_CCR_INSTRUCTION_SHIFT,
          (regval & OCTOSPI_CCR_IMODE_MASK) >> OCTOSPI_CCR_IMODE_SHIFT,
          (regval & OCTOSPI_CCR_ADMODE_MASK) >> OCTOSPI_CCR_ADMODE_SHIFT,
          (regval & OCTOSPI_CCR_ADSIZE_MASK) >> OCTOSPI_CCR_ADSIZE_SHIFT,
          (regval & OCTOSPI_CCR_ABMODE_MASK) >> OCTOSPI_CCR_ABMODE_SHIFT,
          (regval & OCTOSPI_CCR_ABSIZE_MASK) >> OCTOSPI_CCR_ABSIZE_SHIFT,
          (regval & OCTOSPI_CCR_DCYC_MASK) >> OCTOSPI_CCR_DCYC_SHIFT,
          (regval & OCTOSPI_CCR_DMODE_MASK) >> OCTOSPI_CCR_DMODE_SHIFT,
          (regval & OCTOSPI_CCR_FMODE_MASK) >> OCTOSPI_CCR_FMODE_SHIFT,
          (regval & OCTOSPI_CCR_SIOO) ? 1 : 0,
          (regval & OCTOSPI_CCR_DDRM) ? 1 : 0);

  regval = getreg32(priv->base + STM32_OCTOSPI_SR_OFFSET);    /* Status Register */
  spiinfo("SR:%08x\n", regval);
  spiinfo("  TEF:%1d TCF:%1d FTF:%1d SMF:%1d TOF:%1d BUSY:%1d FLEVEL:%d\n",
          (regval & OCTOSPI_SR_TEF) ? 1 : 0,
          (regval & OCTOSPI_SR_TCF) ? 1 : 0,
          (regval & OCTOSPI_SR_FTF) ? 1 : 0,
          (regval & OCTOSPI_SR_SMF) ? 1 : 0,
          (regval & OCTOSPI_SR_TOF) ? 1 : 0,
          (regval & OCTOSPI_SR_BUSY) ? 1 : 0,
          (regval & OCTOSPI_SR_FLEVEL_MASK) >> OCTOSPI_SR_FLEVEL_SHIFT);

#else
  spiinfo("    CR:%08x   DCR:%08x   CCR:%08x    SR:%08x\n",
          getreg32(priv->base + STM32_OCTOSPI_CR_OFFSET),     /* Control Register */
          getreg32(priv->base + STM32_OCTOSPI_DCR_OFFSET),    /* Device Configuration Register */
...
#if defined(CONFIG_STM32H5_OCTOSPI_INTERRUPTS)
                           ...
                           uint8_t fctn)
{
  ...
  regval  = OCTOSPI_CCR_INST(xctn->instr) |
            OCTOSPI_CCR_IMODE(xctn->instrmode) |
            ...
            OCTOSPI_CCR_DCYC(xctn->dummycycles) |
            OCTOSPI_CCR_DMODE(xctn->datamode) |
            OCTOSPI_CCR_FMODE(fctn) |
            (xctn->isddr ? OCTOSPI_CCR_SIOO : 0) |
            (xctn->issioo ? OCTOSPI_CCR_DDRM : 0);
...
#elif defined(CONFIG_STM32H5_OCTOSPI_DMA)
/****************************************************************************
                           ...
                           struct qspi_xctnspec_s *xctn)
{
  ...
  if (QSPIMEM_ISWRITE(meminfo->flags))
    {
      ...
      dmaflags = (OCTOSPI_DMA_PRIO | DMA_SCR_MSIZE_8BITS |
  ...
  stm32_dmasetup(priv->dmach, qspi_regaddr(priv, STM32_OCTOSPI_DR_OFFSET),
  ...
  if (dest != NULL)
    {
      ...
      regval  = qspi_getreg(priv, STM32_OCTOSPI_CCR_OFFSET);
      regval &= ~OCTOSPI_CCR_FMODE_MASK;
      regval |= OCTOSPI_CCR_FMODE(CCR_FMODE_INDRD);
      qspi_putreg(priv, regval, STM32_OCTOSPI_CCR_OFFSET);
...
static uint32_t qspi_setfrequency(struct qspi_dev_s *dev, uint32_t frequency)
{
   ...
   * frequency.
   *
   *   QSCK frequency = OCTOSPI_CLK_FREQUENCY / prescaler, or
   *     prescaler = OCTOSPI_CLK_FREQUENCY / frequency
   *
  ...
  prescaler = (frequency + OCTOSPI_CLK_FREQUENCY - 1) / frequency;
  ...
  regval  = qspi_getreg(priv, STM32_OCTOSPI_CR_OFFSET);
  regval &= ~(OCTOSPI_CR_PRESCALER_MASK);
  regval |= (prescaler - 1) << OCTOSPI_CR_PRESCALER_SHIFT;
  qspi_putreg(priv, regval, STM32_OCTOSPI_CR_OFFSET);

  /* Calculate the new actual frequency */

  actual = OCTOSPI_CLK_FREQUENCY / prescaler;
  ...
  if (mode != priv->mode)
    {
      ...
      regval  = qspi_getreg(priv, STM32_OCTOSPI_DCR_OFFSET);
      regval &= ~(OCTOSPI_DCR_CKMODE);
      ...
      qspi_putreg(priv, regval, STM32_OCTOSPI_DCR_OFFSET);
                       ...
                       struct qspi_meminfo_s *meminfo)
{
  ...
  if (priv->candma &&
      meminfo->buflen > CONFIG_STM32H5_OCTOSPI_DMATHRESHOLD &&
 ...
 *
 * Description:
 *   Free memory returned by OCTOSPI_ALLOC
 *
 * Input Parameters:
 *   dev    - Device-specific state data
 *   buffer - Buffer previously allocated via OCTOSPI_ALLOC
 *
 * Returned Value:
 *   None.
 *
...
static int qspi_hw_initialize(struct stm32_qspidev_s *priv)
{
  ...
  regval &= ~(OCTOSPI_CR_TEIE | OCTOSPI_CR_TCIE | OCTOSPI_CR_FTIE | OCTOSPI_CR_SMIE |
              OCTOSPI_CR_TOIE | OCTOSPI_CR_FSEL | OCTOSPI_CR_DFM);

#if defined(CONFIG_STM32H5_OCTOSPI_MODE_BANK2)
  regval |= OCTOSPI_CR_FSEL;
#endif

#if defined(CONFIG_STM32H5_OCTOSPI_MODE_DUAL)
  regval |= OCTOSPI_CR_DFM;
#endif

  /* Configure QSPI FIFO Threshold */

  regval &= ~(OCTOSPI_CR_FTHRES_MASK);
  regval |= ((CONFIG_STM32H5_OCTOSPI_FIFO_THESHOLD - 1) <<
  ...
  regval  = qspi_getreg(priv, STM32_OCTOSPI_CR_OFFSET);
  regval &= ~(OCTOSPI_CR_PRESCALER_MASK | OCTOSPI_CR_SSHIFT);
  regval |= (0x01 << OCTOSPI_CR_PRESCALER_SHIFT);
  regval |= (0x00);
  qspi_putreg(priv, regval, STM32_OCTOSPI_CR_OFFSET);
  ...
  regval  = qspi_getreg(priv, STM32_OCTOSPI_DCR_OFFSET);
  regval &= ~(OCTOSPI_DCR_CKMODE | OCTOSPI_DCR_CSHT_MASK | OCTOSPI_DCR_FSIZE_MASK);
  regval |= (0x00);
  regval |= ((CONFIG_STM32H5_OCTOSPI_CSHT - 1) << OCTOSPI_DCR_CSHT_SHIFT);
  if (0 != CONFIG_STM32H5_OCTOSPI_FLASH_SIZE)
    {
      unsigned int nsize = CONFIG_STM32H5_OCTOSPI_FLASH_SIZE;
      int nlog2size = 31;
      ...
      regval |= ((nlog2size - 1) << OCTOSPI_DCR_FSIZE_SHIFT);
    }
  ...
  if (intf == 0)
    {
      ...
      stm32_configgpio(GPIO_OCTOSPI_CS);
      stm32_configgpio(GPIO_OCTOSPI_IO0);
      stm32_configgpio(GPIO_OCTOSPI_IO1);
      stm32_configgpio(GPIO_OCTOSPI_IO2);
      stm32_configgpio(GPIO_OCTOSPI_IO3);
      stm32_configgpio(GPIO_OCTOSPI_SCK);
    }
  else
    {
  ...
  if (!priv->initialized)
    {
      ...
      if (priv->candma)
        {
          priv->dmach = stm32_dmachannel(DMACHAN_OCTOSPI);
          if (!priv->dmach)
            {
