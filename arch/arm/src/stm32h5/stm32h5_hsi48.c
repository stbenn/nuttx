/****************************************************************************
 * arch/arm/src/stm32h5/stm32h5_hsi48.c
 *
...
#include "hardware/stm32h5_crs.h"

#include "stm32h5_hsi48.h"
...
void stm32h5_enable_hsi48(enum syncsrc_e syncsrc)
{
  ...
  while ((getreg32(STM32H5_RCC_CRRCR) & RCC_CRRCR_HSI48RDY) == 0);
  ...
  regval = getreg32(STM32H5_CRS_CFGR);
  ...
  putreg32(regval, STM32H5_CRS_CFGR);
  ...
  regval  = getreg32(STM32H5_CRS_CR);
  regval |= CRS_CR_AUTOTRIMEN | CRS_CR_CEN;
  putreg32(regval, STM32H5_CRS_CR);
}
