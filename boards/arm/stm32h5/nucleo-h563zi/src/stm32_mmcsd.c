/****************************************************************************
 * boards/arm/stm32h5/nucleo-h563zi/src/stm32_mmcsd.c
 *
...
#ifndef CONFIG_STM32H5_SPI1
#  error "MMC/SD requires SPI1 enabled"
#endif
...
/*static const int MMCSD_SPI_PORT = 1;  SD is connected to SPI1 port */
static const int MMCSD_SLOT_NO = 0;  /* There is only one SD slot */
...
int stm32_mmcsd_initialize(int minor)
{
  ...
  if ((spi = stm32_spibus_initialize(MMCSD_SPI_PORT)) == NULL)
    {
      ferr("failed to initialize SPI port %d\n", MMCSD_SPI_PORT);
      return -ENODEV;
    }

  if ((rv = mmcsd_spislotinitialize(minor, 0, spi)) < 0)
    {
 ...
 /* stm32_gpiosetevent(GPIO_MMCSD_NCD), true, true, true, stm32_cd, NULL); 
