...
uint32_t board_button_initialize(void)
{
  ...
  stm32h5_configgpio(GPIO_BTN_USER);
  return NUM_BUTTONS;
}
...
uint32_t board_buttons(void)
{
  ...
  return stm32h5_gpioread(GPIO_BTN_USER) ? BUTTON_USER_BIT : 0;
}
...
int board_button_irq(int id, xcpt_t irqhandler, void *arg)
{
  ...
  if (id == BUTTON_USER)
    {
      ret = stm32h5_gpiosetevent(GPIO_BTN_USER, true, true, true, irqhandler,
                                 arg);
    }
